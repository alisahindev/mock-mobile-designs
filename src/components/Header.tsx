...
export default function Header({ userName, avatarUrl }: HeaderProps) {
  ...
  const allowed = (
    pathname === '/' || pathname === '/completed' || pathname === '/progress' ||
  ...
  return (
    <header className="flex items-center justify-between px-4 pt-safe-8 pb-4" role="banner" aria-label="Üst bilgi">
      ...
      >
        ...
        <span className="absolute -top-0.5 left-3 w-2 h-2 rounded-full" style={{ backgroundColor: 'var(--primary)' }} aria-hidden="true" />
      </button>
    </header>
  )
}
